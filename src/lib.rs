...
impl Default for BuildMode {
	fn default() -> Self { Self::CArchive }
}

/// A builder for compilation of a native golang project.
///
/// A `Build` is the main type of the `gobuild` crate and is used to control all
/// the various configuration options and such of a compile. You'll find more
...
impl Build {
	...
	/// Run the compiler, generating the file `output`
	///
	/// The name `output` should be the name of the library. The Rust compilier
	/// will create the assembly with the lib prefix and .a extension.
	///
	/// # Panics
	///
	...
	fn get_goarch(&self) -> Result<OsString, Error> {
		...
		// let endian = env::var("CARGO_CFG_TARGET_ENDIAN").map_err(|_|
		// Error::new(ErrorKind::EnvVarNotFound, "Cannot find CARGO_CFG_TARGET_ENDIAN
		// env var"))?;
